	...
	if(isset($_POST['Submit'])){
		...
		if(!empty($_POST['Name']) && !empty($_POST['Email'])){
			if((preg_match($NamePattern,$_POST['Name']) == true) && (preg_match($EmailPattern,$_POST['Email']) == true) && (preg_match($NumberPattern,$_POST['Number']) == true)){
				...
				$query = "INSERT INTO hosts(name,email,phone) VALUES('$Name','$Email',$Number)";

				$execute_query = mysqli_query($connection,$query);
				
            	if($execute_query){
					$_SESSION["SuccessMessage"] = "Successfully, added as a Host.";
...
<div class="container">
    <!--- Host Form -->
    <form action="host.php" method="POST" class="col-sm-9">
